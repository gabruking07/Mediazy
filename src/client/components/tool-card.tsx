...
export function ToolCard({ tool }: { tool: ToolModule }) {
  ...
  return (
    <Link href={`/tools/${tool.slug}`} className="group block">
      <Card className="relative h-full overflow-hidden rounded-lg border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-violet-200 hover:shadow-premium">
        <div className={`absolute inset-x-0 top-0 h-28 bg-gradient-to-br ${categoryStyles[tool.category]}`} />
        <CardHeader>
          <div className="flex items-start justify-between gap-3">
            <span className="relative grid size-12 place-items-center rounded-lg border border-slate-100 bg-white shadow-sm">
